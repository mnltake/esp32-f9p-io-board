...
for(;;){ // MAIN LOOP FOR THIS CORE
  ...
  if (IOSettings.enableNtrip==1 && my_WiFi_Mode == WIFI_STA){
     if (restart == 0){
  ...
  delay(1);
  if(IOSettings.enableNtrip == 2 && aogntriplife <= millis()) {
    DBG("\nRelaying Ntrip Data from AOG: ");
    aogntriplife = millis()+5000;
   }
  if (IOSettings.enableNtrip == 0){
...
bool startStream(){
 ...
 // This will send the request to the server
 String requestMtp =(String("GET /") + IOSettings.mountpoint + " HTTP/1.0\r\n" +
                      "User-Agent: " + _userAgent + "\r\n");
 if (strlen(IOSettings.ntripUser)==0){
 ...
 if (IOSettings.sendGGAsentence > 0) {
    sendGGA(); // 
    repeatGGA = millis();
    DBG("", 1); //NL
  }
...
bool getRtcmData(){
    ...
    while (ntripCl.available() == 0) {
        if (millis() - timeout > 5000) {
            ...
            if(sizeof(lastSentence) < 50 && (IOSettings.sendGGAsentence >1)) DBG("Invalid NMEA String from GPS\n");
            if(IOSettings.sendGGAsentence == 1) DBG("Maybe invalid fixed NMEA String\n");
            if(IOSettings.sendGGAsentence == 0) DBG("Check if your Provider requires your Position\n");
            return false;
        }
      delay(1); // WDT prevent
     }
   ...
   while(ntripCl.available()&& IOSettings.enableNtrip == 1) {
      char a = ntripCl.read();
      Serial1.print(a);
      if (IOSettings.sendGGAsentence > 0){
        if (millis() - repeatGGA > (IOSettings.GGAfreq * 1000)) {
          sendGGA(); //
          repeatGGA = millis();
        }
      }
...
bool connectCaster(){
  ...
  setAuthorization(IOSettings.ntripUser, IOSettings.ntripPassword); // Create Auth-Code
 
  return ntripCl.connect(IOSettings.host, IOSettings.port);
}
//###########################################################################################
char* readLine(){
  int i=0;
...
void sendGGA(){
    ...
    if (IOSettings.sendGGAsentence == 1){
       ntripCl.print(IOSettings.GGAsentence);
       ntripCl.print("\r\n");
      }
    if (IOSettings.sendGGAsentence > 1){
